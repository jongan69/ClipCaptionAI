...
import {outputsRoot, ensureDir, projectRoot, workspaceRoot, emitJsonResult} from './lib.mjs';
...
export const describeAsset = (filePath, root = projectRoot) => ({
  ...
  type: ['.mp4', '.mov', '.m4v', '.webm'].includes(path.extname(filePath).toLowerCase())
    ? 'video'
    : ['.mp3', '.wav', '.m4a', '.aac'].includes(path.extname(filePath).toLowerCase())
      ? 'audio'
      : 'image',
...
export const emitResult = (result, json = false) => {
  if (json) emitJsonResult(result, true);
