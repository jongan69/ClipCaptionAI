...
const requireOne = (...keys) => {
  ...
  if (!key)
    throw new Error(
      `Missing one of ${keys.map((keyName) => `--${keyName}`).join(', ')}.\n${usage}`,
    );
...
const main = async () => {
  ...
  if (
    blueprintsDir &&
    (!fs.existsSync(blueprintsDir) || !fs.statSync(blueprintsDir).isDirectory())
  ) {
  ...
  const baseDir = startingPreviewManifest ? path.dirname(startingPreviewManifest) : blueprintsDir;
  const outDir = path.resolve(
    String(args['out-dir'] ?? path.join(baseDir, 'competitive-control-loop')),
  );
  ensureDir(outDir);

  const previewManifest =
    startingPreviewManifest ?? path.join(blueprintsDir, 'competitive-preview-render-manifest.json');
  const qaReport = path.join(path.dirname(previewManifest), 'competitive-video-qa-report.json');
  const premiumPlanDir = path.join(
    path.dirname(previewManifest),
  ...
  const higgsRenderManifest = path.join(
    higgsRenderDir,
  ...
  const creativePacketsManifest = path.join(
    premiumPlanDir,
  ...
  try {
    ...
    if (args['run-higgsfield-renders'] === true) {
      ...
      if (args['higgs-render-model'])
        renderProviderArgs.push('--model', String(args['higgs-render-model']));
      if (args['higgs-render-max-jobs'])
        renderProviderArgs.push('--max-jobs', String(args['higgs-render-max-jobs']));
      ...
      run({
        name: 'render_higgsfield_jobs',
        commandArgs: renderProviderArgs,
        expectedFiles: [higgsRenderManifest, higgsRenderUrlMap],
      });
    }
    ...
    run({
      name: 'collect_generated_clips',
      commandArgs: collectArgs,
      expectedFiles: [collectManifest],
    });
    ...
    run({
      name: args['attempt-finalize'] === true ? 'finalize_ready_ads' : 'finalize_readiness',
      commandArgs: finalizeArgs,
      expectedFiles: [finalizeManifest],
    });
    ...
    run({
      ...
      expectedFiles: [statusReport],
    });

    run({
      name: 'export_creative_packets',
      commandArgs: [
        'scripts/ebay/export-competitive-creative-packets.mjs',
        '--status',
        statusReport,
      ],
      expectedFiles: [creativePacketsManifest],
    });

    run({
      name: 'build_review_board',
      commandArgs: ['scripts/ebay/build-competitive-review-board.mjs', '--status', statusReport],
      expectedFiles: [reviewBoard],
    });
  } catch (error) {
  ...
  fs.writeFileSync(
    ...
    `${[
      ...
      ...steps.map(
        (step) =>
          `- ${step.name}: ${step.status}${step.exit_code === null ? '' : ` (${step.exit_code})`}`,
      ),
      '',
    ].join('\n')}\n`,
  );
