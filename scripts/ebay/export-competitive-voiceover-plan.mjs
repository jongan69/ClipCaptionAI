...
const sellerPitchLines = (blueprint, durationSeconds) => {
  ...
  const category = cleanLine(blueprint.listing?.inferred_category || '').replace(
    /^marketplace product$/i,
    '',
  );
...
const scriptForBlueprint = ({blueprint, durationSeconds, maxWords}) => {
  const originalLines = Array.isArray(blueprint.original_script?.lines)
    ...
    : [];
  const sourceLines =
    originalLines.length > 0 && !shouldReplaceOriginalScript(originalLines)
      ? originalLines
      : sellerPitchLines(blueprint, durationSeconds);
  const finalLines =
    sourceLines.length > 0 ? sourceLines : fallbackLines(blueprint.listing, durationSeconds);
  const text = trimWords(finalLines.join(' '), maxWords);
  const words = text.split(/\s+/).filter(Boolean);
  return {
    style:
      cleanLine(blueprint.original_script?.voiceover_style) ||
...
const main = async () => {
  ...
  const outDir = path.resolve(
    String(args['out-dir'] ?? path.join(blueprintsDir, 'competitive-voiceover-plan')),
  );
  ...
  for (const blueprintPath of findBlueprints(blueprintsDir)) {
    ...
    fs.writeFileSync(
      path.join(packetDir, 'voiceover-entry.json'),
  ...
  fs.writeFileSync(
    path.join(outDir, 'render-voiceover-previews.sh'),
    `${[
      '#!/usr/bin/env bash',
      'set -euo pipefail',
