...
const requireAny = (...keys) => {
  ...
  if (!key)
    throw new Error(`Missing one of ${keys.map((value) => `--${value}`).join(', ')}.\n${usage}`);
...
const main = async () => {
  ...
  const previewManifestPath = args['preview-manifest']
  ...
  const inferredPreview =
    previewManifestPath ??
    inferSibling({
      baseFile: premiumPlanPath,
      relative: '../competitive-preview-render-manifest.json',
    });
  ...
  const inferredCollect = args['collect-manifest']
    ...
    : inferSibling({
        baseFile: premiumPlanPath,
        relative: 'competitive-premium-collect-manifest.json',
      });
  const inferredFinalize = args['finalize-manifest']
    ? path.resolve(String(args['finalize-manifest']))
    : inferSibling({
        baseFile: premiumPlanPath,
        relative: 'competitive-premium-finalize-manifest.json',
      });
  const inferredHandoff = args['handoff-manifest']
    ? path.resolve(String(args['handoff-manifest']))
    : inferSibling({
        baseFile: premiumPlanPath,
        relative: 'competitive-render-handoff/competitive-render-handoff-manifest.json',
      });
  ...
  const decideStatus = (item) => {
    ...
    if (item.premium?.job_count > 0) {
      ...
      if (missing.length > 0)
        blockers.push(...missing.map((job) => `Missing generated clip: ${job.output_hint}`));
    }
    ...
    if (item.premium_hold?.reference_quality?.status === 'research_review_required') {
      ...
      blockers.push(
        ...(item.premium_hold.reference_quality.issues ?? []).map(
          (issue) => `Research quality: ${issue}`,
        ),
      );
    }
    ...
    if (collectionJobs.length > 0) {
      ...
      if (failed.length > 0) {
        ...
        nextAction =
          'Provide Higgsfield output URLs/files, then rerun ebay:collect-premium-renders.';
        blockers.push(
          ...missingSource.map((job) => `No source for ${job.job_id}: ${job.output_hint}`),
        );
      }
    }
    ...
    if (item.finalization) {
      if (item.finalization.assembled && item.finalization.probe?.has_video) {
        ...
        blockers.push(
          ...(item.finalization.missing_clips ?? []).map(
            (clip) => `Missing generated clip: ${clip.path}`,
          ),
        );
      }
    }
  ...
  const outFile = path.resolve(
    String(args.out ?? path.join(path.dirname(baseFile), 'competitive-video-pipeline-status.json')),
  );
    ...
    ...items.flatMap((item) =>
      [
        ...
        ...(item.blockers.length
          ? ['- Blockers:', ...item.blockers.map((blocker) => `  - ${blocker}`)]
          : []),
        '',
      ].filter(Boolean),
    ),
