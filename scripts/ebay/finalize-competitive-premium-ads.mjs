...
const usage = `
Usage:
  bun scripts/finalize-competitive-premium-ads.mjs --premium-plan outputs/.../competitive-premium-render-plan.json
  bun run ebay:finalize-premium-ads -- --premium-plan outputs/.../competitive-premium-render-plan.json
...
const expectedClipsForPacket = (packet) =>
  (packet.jobs ?? []).map((job) => ({id: job.id, path: job.output_hint})).filter((job) => job.path);
...
const buildAssembleArgs = (packet) => {
  ...
  for (const key of ['max-broll-clips', 'max-broll-seconds', 'max-clip-seconds']) {
    if (args[key] !== undefined && args[key] !== false)
      assembleArgs.push(`--${key}`, String(args[key]));
  }
...
const main = async () => {
  ...
  const outManifest = path.resolve(
    String(
      args['out-manifest'] ??
        path.join(path.dirname(premiumPlanPath), 'competitive-premium-finalize-manifest.json'),
    ),
  );
  ...
  for (const packet of premiumPlan.selected ?? []) {
    ...
    if (entry.final_manifest) {
      ...
      entry.assembled = Boolean(
        entry.final_video && fs.existsSync(entry.final_video) && entry.probe?.has_video,
      );
    }
    if (!entry.assembled)
      entry.error = 'Assembler completed but final video verification did not pass.';
    results.push(entry);
  }
  ...
  const manifest = {
    ...
    failed_count: results.filter(
      (entry) => entry.ready && !entry.assembled && args['dry-run'] !== true,
    ).length,
    source_policy:
      'Final assembly requires real generated/owned clips in higgsfield-renders; no slideshow fallback is allowed.',
    results,
  };
