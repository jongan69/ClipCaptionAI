...
const relativeOrNull = (from, file) =>
  file ? path.relative(from, file).split(path.sep).join('/') : null;
...
const writeResearchBrief = ({item, blueprint, packetDir}) => {
  if (item.status !== 'research_review_required' && !item.premium_hold?.reference_quality)
    return null;
  ...
  const sourcePrompt = item.blueprint
    ...
    : null;
  const promptText =
    sourcePrompt && fs.existsSync(sourcePrompt) ? fs.readFileSync(sourcePrompt, 'utf8') : null;
  ...
  fs.writeFileSync(
    path.join(researchDir, 'research-brief.md'),
    `${[
      ...
      '',
      ...(
        item.premium_hold?.reference_quality?.issues ?? ['Needs stronger competitor evidence.']
      ).map((issue) => `- ${issue}`),
      '',
      '## Search Queries',
      '',
...
const markdownForPacket = ({item, blueprint, qa, local, packetDir}) => {
  ...
  const lines = [
    ...
    '',
    local.preview_video
      ? `- Preview video: ${relativeOrNull(packetDir, local.preview_video)}`
      : '- Preview video: not copied',
    local.proof_frame
      ? `- Proof frame: ${relativeOrNull(packetDir, local.proof_frame)}`
      : '- Proof frame: not copied',
    ...local.reference_images.map(
      (image) => `- Reference image: ${relativeOrNull(packetDir, image)}`,
    ),
    '',
    '## Preview QA',
    '',
    qa
      ? [
          ...
          `Score: ${qa.score}`,
          qa.probe
            ? `Probe: ${qa.probe.width}x${qa.probe.height}, ${qa.probe.duration_seconds}s, audio=${qa.probe.has_audio}`
            : null,
          qa.audio?.mean_volume_db !== undefined
            ? `Mean audio: ${qa.audio.mean_volume_db} dB`
            : null,
          qa.scenes?.scene_change_count !== undefined
            ? `Scene changes: ${qa.scenes.scene_change_count}`
            : null,
          ...(qa.issues?.length ? ['Issues:', ...qa.issues.map((issue) => `- ${issue}`)] : []),
          ...(qa.warnings?.length
            ? ['Warnings:', ...qa.warnings.map((warning) => `- ${warning}`)]
            : []),
        ]
          .filter(Boolean)
          .join('\n')
...
const main = async () => {
  ...
  const outDir = path.resolve(
    String(args['out-dir'] ?? path.join(path.dirname(statusPath), 'competitive-creative-packets')),
  );
  ensureDir(outDir);
  ...
  for (const item of status.items ?? []) {
    ...
    const previewVideo = shouldCopyPreview
      ? copyIfExists({
          source: item.preview?.final_video,
          dest: path.join(previewDir, path.basename(item.preview?.final_video ?? 'preview.mp4')),
          missing,
        })
      : null;
    const proofFrame = shouldCopyPreview
      ? copyIfExists({
          source: item.preview?.proof_frame,
          dest: path.join(
            previewDir,
            path.basename(item.preview?.proof_frame ?? 'proof-frame.jpg'),
          ),
          missing,
        })
      : null;
    ...
    const queue = {
      jobs: (item.handoff?.jobs ?? []).map((job) => ({
        ...job,
        item_id: item.item_id,
        title: item.title,
      })),
    ...
    const urlMap = Object.fromEntries(
      queue.jobs.map((job) => [job.job_id ?? job.id, job.output_hint]),
    );
    ...
    fs.writeFileSync(
      path.join(packetDir, 'creative-packet.md'),
      markdownForPacket({
        item,
        blueprint,
        qa,
  ...
  console.log(
    `Competitive creative packets: ${path.join(outDir, 'competitive-creative-packets-manifest.json')}`,
  );
