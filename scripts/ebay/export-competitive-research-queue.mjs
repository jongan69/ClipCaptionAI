...
const main = async () => {
  ...
  if (!fs.existsSync(packetsManifestPath)) {
    throw new Error(
      `Creative packets manifest not found: ${packetsManifestPath}. Run ebay:competitive-packets first.`,
    );
  }
  ...
  const outDir = path.resolve(
    String(
      args['out-dir'] ??
        path.join(path.dirname(packetsManifestPath), '..', 'competitive-research-queue'),
    ),
  );
  ensureDir(outDir);
  ...
  for (const packetSummary of packetsManifest.packets ?? []) {
    ...
    if (
      !packetJson ||
      !fs.existsSync(packetJson) ||
      !researchBriefJson ||
      !fs.existsSync(researchBriefJson)
    ) {
      continue;
    }
    ...
    if (
      packet.status !== 'research_review_required' &&
      research.status !== 'research_review_required'
    ) {
      continue;
    }
