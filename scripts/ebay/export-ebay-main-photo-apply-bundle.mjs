...
const main = async () => {
  ...
  const outDir = path.resolve(
    String(args['out-dir'] ?? path.join(path.dirname(queuePath), 'main-photo-apply-bundle')),
  );
  ...
  const ready = (Array.isArray(queue.entries) ? queue.entries : [])
    .filter(
      (entry) => entry.final_status === 'ready_for_upload_preview' && entry.selected_main_photo,
    )
  ...
  for (const entry of ready) {
    ...
    const itemDir = path.join(
      outDir,
      `${String(entry.rank).padStart(2, '0')}-${itemId}-${safeName(entry.title)}`,
    );
    ensureDir(itemDir);
    ...
    fs.writeFileSync(
      path.join(itemDir, 'mcp-preview-payload.json'),
      `${JSON.stringify(previewPayload, null, 2)}\n`,
    );
    fs.writeFileSync(
      path.join(itemDir, 'mcp-apply-payload.json'),
  ...
  const manifest = {
    ...
    live_apply_blocker:
      queue.live_apply_blocker ?? 'Live apply requires authenticated eBay seller session.',
    safety: {
      price_changes: false,
  ...
  const readme = [
    ...
    ...entries.map(
      (entry) =>
        `| ${entry.rank} | [${entry.item_id}](${entry.url}) ${String(entry.title).replaceAll('|', '-')} | ${(Number(entry.ctr ?? 0) * 100).toFixed(2)}% | ${entry.upload_image ?? 'MISSING'} |`,
    ),
    '',
  ].join('\n');
