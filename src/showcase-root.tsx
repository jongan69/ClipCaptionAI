...
export const ShowcaseRoot = () => (
  <>
    ...
    {compositionMatrix().map((entry) => {
      const Stage: ComponentType<LogoCompositionProps> = ({background}) => (
        <LogoStage logo={entry.logo} variant={entry.component} background={background} />
      );
      return (
        <Composition
          key={entry.id}
          ...
          defaultProps={{background: entry.logo.brand.palette.background ?? '#000'}}
        />
      );
    })}
