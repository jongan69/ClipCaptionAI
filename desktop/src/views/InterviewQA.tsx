...
type PipelineStage =
  'drop' | 'probing' | 'ready' | 'transcribing' | 'analyzing' | 'review' | 'rendering' | 'done';
...
export default function InterviewQA() {
  ...
  const handleVideoSelected = useCallback(
    async (filePath: string, fileName: string, fileSize: number) => {
      ...
      try {
        ...
        const result = await window.cca?.probeVideo?.(filePath);
        if (result) {
  ...
  const handleStartAnalysis = useCallback(async () => {
    ...
    try {
      ...
      const result = await window.cca?.runWorkflow({
    ...
    const unsub = window.cca?.onComplete(async (payload: any) => {
      unsub?.();
      ...
      try {
        const qaResult = await window.cca?.runWorkflow({
          workflowId: 'interview-qa',
          argValues: {video: video.path},
        });
        if (qaResult?.session) setSessionId(qaResult.session);
      } catch {
      ...
      const unsub2 = window.cca?.onComplete(async (payload2: any) => {
        unsub2?.();
        setProgress(100);
  ...
  const generateSampleSegments = async (_videoPath: string) => {
    ...
    const sampleSegments: QAPair[] = [
      {
        ...
        answer:
          "I've always been passionate about solving this problem. When I saw how much time people were spending on manual video editing, I knew there had to be a better way. We started by building a simple prototype that could automatically detect the best moments in a video, and it just grew from there.",
      ...
      },
      {
        ...
        answer:
          "The hardest part was getting the AI to understand context. Early versions would cut in the middle of sentences or miss the emotional arc of a story. We spent months refining the thought-boundary detection and viral scoring. Now it's extremely reliable — it catches about 95% of good moments automatically.",
        speakerQ: 'Interviewer',
        speakerA: 'Guest',
        startSeconds: 44.1,
        endSeconds: 78.3,
      ...
      },
      {
        ...
        answer:
          "Just start. Ship something small. The first version of our product was embarrassingly basic — it was literally just a Python script that called ffmpeg with hardcoded settings. But we put it out there, got feedback, and iterated. If I had waited until it was 'ready,' I'd still be coding version one.",
        speakerQ: 'Interviewer',
        speakerA: 'Guest',
        startSeconds: 130.2,
        endSeconds: 165.0,
  ...
  const loadQAResults = async () => {
    ...
    try {
      const content = await window.cca?.readProjectFile?.('interview-qa-output.json');
      if (content) {
      ...
      /* missing or unreadable — fall back to sample below */
    }
    // Fall back to sample
  ...
  const handleRender = useCallback(async () => {
    ...
    for (let i = 0; i < selected.length; i++) {
      ...
      try {
        ...
        /* best-effort render — continue with remaining clips */
      }
    }
  ...
  const toggleSegment = (index: number) => {
    setSegments((prev) => prev.map((s) => (s.index === index ? {...s, included: !s.included} : s)));
  };
      ...
      <div className="max-w-4xl mx-auto mt-4 space-y-4">
        ...
        <GlassPanel variant="strong" size="lg">
          ...
          <p className="text-sm text-text-dim">
            Drop a 2-person interview video — auto-detect questions & answers, then render
            individual clips.
          </p>
        </GlassPanel>

        {/* Drop Zone */}
        {stage === 'drop' && <DragDropZone onVideoSelected={handleVideoSelected} />}
        ...
        {(stage === 'transcribing' || stage === 'analyzing') && (
          <GlassPanel className="p-6 text-center space-y-4">
            <div className="text-4xl animate-pulse">{stage === 'transcribing' ? '🎙️' : '🧠'}</div>
            <GlassProgress value={progress} indeterminate={progress === 0} label={progressLabel} />
              ...
              <div className="flex items-center justify-between mb-4">
                <div>
                  ...
                  <p className="text-xs text-text-dim">
                    ...
                    {(Math.floor(totalDuration) % 60).toString().padStart(2, '0')} total
                  </p>
                </div>
                <div className="flex gap-2">
                  <GlassButton
                    size="sm"
                    onClick={() => setSegments((p) => p.map((s) => ({...s, included: true})))}
                  >
                    Select All
                  </GlassButton>
                  <GlassButton
                    size="sm"
                    onClick={() => setSegments((p) => p.map((s) => ({...s, included: false})))}
                  >
                    Deselect All
                  </GlassButton>
                </div>
