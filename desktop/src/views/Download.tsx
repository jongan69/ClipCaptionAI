...
export default function Download() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const content = await window.cca?.readProjectFile('links.txt');
  ...
  const saveLinks = async () => {
    try {
      await window.cca?.writeProjectFile('links.txt', links);
    } catch {
      /* best-effort save */
    }
  };
    ...
    <div className="flex-1 overflow-y-auto">
      <GlassPanel variant="strong" size="lg" className="max-w-2xl mx-auto mt-4">
        ...
        <p className="text-sm text-text-dim mb-6">
          Paste YouTube URLs (one per line) to download videos.
        </p>
        <div className="space-y-4">
          ...
          {running && (
            <GlassProgress
              ...
              indeterminate
              label={currentJob?.stage || 'Downloading...'}
            />
          )}
          <div className="flex gap-3 pt-2">
            <GlassButton
              variant="primary"
              disabled={!links.trim() || running}
              loading={running}
              onClick={() => {
                saveLinks();
                run('download', 'Download Videos', {links: 'links.txt'});
              }}
            >
