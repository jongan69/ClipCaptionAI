...
export default function WorkflowRunner() {
  ...
  const title = workflow?.title || id?.split("-").map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(" ") || "Workflow";
  ...
  const renderField = (arg: WorkflowArg) => {
      ...
      return (
        <div key={arg.name} className="flex gap-2 items-end">
          <GlassInput
            ...
            className="flex-1"
          />
          <GlassButton size="md" onClick={async () => {
    ...
    if (arg.type === "textarea") {
      return (
        <div key={arg.name} className="flex flex-col gap-1.5">
          <label className="text-xs text-text-dim font-medium">{arg.label}{arg.required ? " *" : ""}</label>
          <textarea
            placeholder={arg.placeholder}
            value={formValues[arg.name] || ""}
            onChange={(e) => setValue(arg.name, e.target.value)}
    ...
    return (
      <GlassInput
        ...
        placeholder={arg.placeholder}
        value={formValues[arg.name] ?? ""}
        onChange={(e) => setValue(arg.name, arg.type === "number" ? Number(e.target.value) : e.target.value)}
      />
    );
  };
  ...
  return (
    <div className="flex-1 overflow-y-auto">
      <GlassPanel variant="strong" size="lg" className="max-w-2xl mx-auto mt-4">
        ...
        <p className="text-sm text-text-dim mb-1">{workflow?.description || "Configure and run this workflow."}</p>
        <p className="text-xs text-text-muted font-mono mb-6">clipcaptionai {workflow?.command || id}</p>

        {workflow?.args && workflow.args.length > 0 ? (
          <div className="space-y-4 mb-6">
            {workflow.args.map(renderField)}
          </div>
        ) : (
