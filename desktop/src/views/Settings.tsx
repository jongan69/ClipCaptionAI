...
import {useSecrets, useEnvironment} from '../hooks/useIpc';
...
export default function Settings() {
  ...
  const handleCleanup = async (scope: string) => {
    ...
    try {
      const result = await window.cca?.cleanup(scope);
      setCleanupResult(
        result?.ok
          ? `Cleaned: ${result.cleaned?.join(', ') || 'nothing'}`
          : `Error: ${result?.error}`,
      );
    } catch (e: any) {
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              {[
                {
                  id: 'temp',
                  label: 'Temporary work files',
                  desc: 'Remove temp render and work files',
                },
                {
                  id: 'media-staging',
                  label: 'Media staging cache',
                  desc: 'Clear cached media copies',
                },
                {
                  id: 'old-outputs',
                  label: 'Old output folders',
                  desc: 'Keep last 5 runs, delete older ones',
                },
              ].map((item) => (
                <div
                  key={item.id}
        ...
        {activeTab === 'environment' && (
          <div className="space-y-3">
            {env ? (
              <>
                <div
                  className={`p-3 rounded-xl text-sm ${env.environment?.passed !== false ? 'bg-accent-green/10 border border-accent-green/30 text-accent-green' : 'bg-accent-rose/10 border border-accent-rose/30 text-accent-rose'}`}
                >
                  {env.environment?.passed !== false
                    ? '✅ All required dependencies available'
                    : '❌ Missing required dependencies'}
                </div>
                {env.environment?.required?.length > 0 && (
                  <div className="text-xs text-accent-rose">
                    Missing required: {env.environment.required.join(', ')}
                  </div>
                )}
                {env.environment?.optional?.length > 0 && (
                  <div className="text-xs text-text-dim">
                    Optional tools not found: {env.environment.optional.join(', ')}
                  </div>
                )}
                <div className="text-xs text-text-muted">
                  Last checked:{' '}
                  {env.updatedAt ? new Date(env.updatedAt).toLocaleString() : 'Unknown'}
                </div>
              </>
            ) : (
