...
export function resolveRuntimePaths() {
  ...
  // In packaged mode, spawned `node` children can only read files that
  // electron-builder unpacked from the asar, so point at app.asar.unpacked
  // (see asarUnpack in package.json — bin/, scripts/, src/, etc.).
...
export function bootstrapAssets(paths) {
  ...
  const copies = [
    {
      src: paths.resourceAssets.captionStyle,
      dest: path.join(paths.configRoot, 'caption-style.json'),
    },
